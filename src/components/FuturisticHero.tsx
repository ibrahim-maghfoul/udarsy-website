...
// ── StatCell ──────────────────────────────────────────────────────────────────
function StatCell({ s, val, last, delay, label }: { s: typeof STATS[0]; val: number; last: boolean; delay: number; label: string }) {
  ...
  return (
    <div style={{ flex:1, padding:'20px 16px', textAlign:'center', borderRight: last ? 'none' : '1px solid rgba(255,255,255,0.055)' }}>
      <div className="dlh-stat-val" style={{ fontSize:26, fontWeight:900, color:'#fff', fontFamily:fHead, letterSpacing:'-0.02em', lineHeight:1, marginBottom:4, animationDelay:`${delay}ms` }}>
        {s.fmt(val)}
      </div>
      <div style={{ fontSize:10.5, color:'rgba(255,255,255,0.38)', fontFamily:fBody }}>
        {label}
      </div>
    </div>
  );
      ...
      <section className="dlh" style={{
        ...
        <div className="dlh-content" style={{ position:'relative', zIndex:10, display:'flex', alignItems:'stretch', flex:1, minHeight:'83vh' }}>
          ...
          <div className="dlh-left" style={{
            ...
            {/* Headline — 3 lines */}
            <div style={{ animation:'hFadeUp 0.7s ease-out 0.25s both', marginBottom:20 }}>
              {([
            ...
            {/* CTAs */}
            <div style={{ display:'flex', alignItems:'center', gap:12, animation:'hFadeUp 0.7s ease-out 0.55s both' }}>
              <button
                onClick={() => router.push('/courses')}
                style={{ display:'flex', alignItems:'center', gap:7, padding:'12px 24px', borderRadius:999, background:'rgba(58,170,106,0.28)', border:'1px solid rgba(58,170,106,0.52)', color:'#fff', fontSize:13.5, fontWeight:700, fontFamily:fBody, cursor:'pointer', boxShadow:'0 4px 22px rgba(58,170,106,0.22)', transition:'transform 0.25s ease, box-shadow 0.25s ease', willChange:'transform', whiteSpace:'nowrap' }}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:9 }}>
                <button
                  ...
                  style={{ width:42, height:42, borderRadius:'50%', background:'rgba(255,255,255,0.055)', border:'1px solid rgba(255,255,255,0.18)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', transition:'background 0.2s ease', flexShrink:0 }}
        ...
        <div style={{ position:'relative', zIndex:15 }}>
          ...
          <div style={{ display:'flex', justifyContent:'center', padding:`0 clamp(20px,3vw,48px) 52px` }}>
            <div ref={statsRef} className={`dlh-stats${statsVisible ? ' dlh-stats-live' : ''}`} style={{ display:'flex', background:'rgba(255,255,255,0.042)', backdropFilter:'blur(28px)', WebkitBackdropFilter:'blur(28px)', border:'1px solid rgba(255,255,255,0.07)', borderTop:'1px solid rgba(255,255,255,0.12)', borderRadius:20, overflow:'hidden', maxWidth:820, width:'100%', boxShadow:'0 -6px 36px rgba(0,0,0,0.22),0 24px 60px rgba(0,0,0,0.42)' }}>
              {STATS.map((s, i) => {
                const labelKey = (['stat_students','stat_courses','stat_scholarships','stat_satisfaction'] as const)[i];
                return <StatCell key={i} s={s} val={countVals[i]} last={i===STATS.length-1} delay={i * 80} label={t(labelKey)}/>;
              })}
            </div>
          </div>
        </div>
